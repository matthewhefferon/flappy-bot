...
const socialLinks = [
  {
    name: "YouTube",
    url: "https://www.youtube.com/@metabasedata",
    icon: (
      <svg
        width="20"
        height="20"
...
export default function SocialLinks({ show }: SocialLinksProps) {
  if (!show) return null;
    ...
    <div className="flex gap-2">
      {socialLinks.map((social) => (
        <Link
          ...
          title={social.name}
        >
          <div className="text-gray-600">{social.icon}</div>
        </Link>
      ))}
    </div>
  );
